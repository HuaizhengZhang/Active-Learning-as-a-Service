...
from alaas.server.util.processor import img_transform


def least_confidence_al(n, path_list, embeddings):
    path_list = np.array(path_list)
    uncertainties = np.amax(embeddings, axis=1)
    return path_list[uncertainties.argsort()[:int(n)]]


class ALServer(Executor):
    def __init__(
            self,
    ...
    def query(self, docs: DocumentArray, parameters, **kwargs):
        ...
        with self.monitor(
            ...
            _doc_list = []
            query_results = least_confidence_al(parameters['budget'], uris, docs.embeddings)
    ...
    def _preproc_images(self, docs: DocumentArray):
        with self.monitor(
            ...
            for data in docs:
                ...
                if self._transform:
                    _tensor_list.append(img_transform(data.tensor))
                else:
